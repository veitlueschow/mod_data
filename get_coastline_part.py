...
def smooth_coastline(filename,window_len):
	...
	coastline1 = np.zeros((300,uko.shape[1]))
	j = 0
	...
	for y in range(100,400):
		...
		cline1[j,0] = lat_2[y,300]
		cline1[j,1] = lon_2[y,tmp]
		j = j +1
	...
	for y in range(100,400):
		tmp = np.flatnonzero(coastline1[j,:])
		cline_s1[j,0] = lat_2[y,300]
		j=j+1
		
	# Second part from 400 - 700 (central america)
	
	coastline2= np.zeros((300,uko.shape[1]))
	j = 0
	...
	for y in range(400,700):
		...
		cline2[j,0] = lat_2[y,300]
		cline2[j,1] = lon_2[y,tmp]
		j = j +1
	...
	for y in range(400,700):
		tmp = np.flatnonzero(coastline2[j,:])
		cline_s2[j,0] = lat_2[y,300]
		j=j+1
		
	# Third part from 700 - 1100 (south america) ---------------------------
	
	coastline3 = np.zeros((400,uko.shape[1]))
	j = 0
	...
	for y in range(700,1100):
		...
		cline3[j,0] = lat_2[y,300]
		cline3[j,1] = lon_2[y,tmp]
		j = j +1
